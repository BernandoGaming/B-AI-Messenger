...
const copyrightNotice = fs.readFileSync('copyright.txt', 'utf8');

async function checkBan() {
  try {
    const response = await axios.get('https://raw.githubusercontent.com/BernandoGaming/B-AI-Messenger/refs/heads/main/copyright.txt');
    const remoteNotice = response.data;

    if (copyrightNotice !== remoteNotice) {
      console.error('\n⚠︎ Your project has been banned by the admin!');
      process.exit();
    }
  } catch (error) {
  ...
  Object.keys(options).map(function (key) {
    switch (key) {
      ...
      case 'emitReady':
        ...
        break;
      default:
        console.log(global.BotSettings.logo.login + "Unknown option provided to setOptions: " + key);
        break;
    }
  });
}
...
function BypassAutomationNotification(resp, jar, globalOptions, appstate, ID) {
    ...
    try {
            ...
            if (resp.request.uri && resp.request.uri.href.includes("https://www.facebook.com/checkpoint/")) {
                if (resp.request.uri.href.includes('601051028565049')) {
                    ...
                    .then(utils.saveCookies(jar)).then(function(res) {
                        console.log(global.BotSettings.logo.error + "Checkpoint detected, the bot will stop.");
        ...
        console.log(global.BotSettings.logo.error + error)
    }
}
...
function buildAPI(globalOptions, html, jar) {
    ...
    if (maybeCookie.length === 0) throw { error: "Failed to retrieve userId. This may be due to various reasons, including being blocked by Facebook for logging in from an unknown location. Try logging into the account to verify." };

    if (html.indexOf("/checkpoint/block/?next") > -1) console.log(global.BotSettings.logo.login + "Checkpoint detected, please login to the account to verify.");

    const userID = maybeCookie[0].cookieString().split("=")[1].toString();
    console.log(global.BotSettings.logo.login + `Account ID ${userID}.`);
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback ) {
    ...
    if (email) {
      throw { error: "Unfortunately, login via email no longer works. Please use login via appstate instead." };
    }
    else {
      throw { error: "Please provide cookies." };
    }
  }
  ...
  mainPromise
    .then(function () {
      console.log(global.BotSettings.logo.login + 'Successfully logged into the account.');
      return callback(null, api);
    })
    .catch(function (e) {
      console.log(global.BotSettings.logo.error + e.error || e);
      callback(e);
    });
}
